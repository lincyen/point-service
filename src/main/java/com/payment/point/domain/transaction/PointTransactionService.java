...
public class PointTransactionService {
    ...
    /**
     * <b>동일 회원, 동일 주문번호, 동일 거래 유형 요청 시 중복체크 valid</b>
     *
     * @param memberId 회원아이디
     * @param orderNo 클라이언트 주문번호
     * @param txType 거래 유형
     */
    public void validateDuplicateOrder(String memberId, String orderNo, TxType txType) {
        if (memberId == null || memberId.isBlank() || orderNo == null || orderNo.isBlank() || txType == null) {
            throw new ApiException(ErrorCode.INVALID_PARAMETER);
        }
        if (pntTrHistRepository.existsByMemberIdAndOrderNoAndTxType(memberId, orderNo, txType)) {
    ...
    public TransactionLookupResponse getTransactionByOrder(String memberId, String orderNo, TxType txType) {
        ...
        Optional<PntTrHist> transaction = pntTrHistRepository.findByMemberIdAndOrderNoAndTxType(memberId, orderNo, txType);
